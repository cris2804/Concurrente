...
public class ServidorMaestro {
    ...
    public static void enviarLibrosAClientes() throws IOException {
        for (Socket nodo : nodos) {
            PrintWriter salida = new PrintWriter(nodo.getOutputStream(), true);
            salida.println("Libro.txt");  // Envía el nombre del libro al nodo
            System.out.println("Enviado libro al nodo: " + nodo.getInetAddress().getHostName());
        }
    }
}

class ManejadorNodo implements Runnable {
    private Socket nodo;
    ...
    public void run() {
        try {
            // Simulación de recepción de modelo entrenado
